#ifndef _cTextureBMP_HG_
#define _cTextureBMP_HG_
...
public:
	cTextureBMP();
	cTextureBMP(const std::string& textureFileName, const std::string& fullPathName, unsigned long index, bool bGenerateMipMap = true);
	~cTextureBMP();
	...
	struct sTextureInfo
	{
		sTextureInfo(const std::string& filename, const std::string& fullPathName, unsigned long index, bool bGenerateMipMap)
		{
			this->_texName = filename;
			this->_fullPathName = fullPathName;
			this->_texIndex = index;
			this->_bHasMipMap = bGenerateMipMap;

			this->_texState = eTexState::PENDING;
			//printf("Texinfo %d created.\n", index);
		}

		~sTextureInfo() { /*printf("TexInfo %d destroyed.\n", this->_texIndex);*/ };

		std::string _texName;
		std::string _fullPathName;
		eTexState _texState;
		unsigned long _texIndex;
		bool _bHasMipMap;
	};
	...
	// Threaded call
	bool LoadTextureFromCPU(const std::string& fileNameFullPath);
	bool LoadTextureIntoGPU(bool bGenerateMIPMap);

	bool CreateTextureThread(cTextureTripleBuffer* pTexTripleBuffer);
	...
	std::string GetTextureFullPathName();

	sTextureInfo* GetTextureInfo();

	bool UpdateTripleBuffer();
...
private:
	...
	sTextureInfo* _pTexInfo;
	cTextureTripleBuffer* _pTexTripleBuffer;
