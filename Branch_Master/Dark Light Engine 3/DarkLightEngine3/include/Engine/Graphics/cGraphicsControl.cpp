...
void cGraphicsControl::CreateGraphics()
{
	...
	this->_pDebugRenderer = new cDebugRenderer();

	this->_pShaderManager->SetBasePath("assets/shaders/");
	this->_pDebugRenderer->Initialize(this->_pShaderManager);
}
...
void cGraphicsControl::DestroyGraphics()
{
	...
	if (this->_pDebugRenderer)
	{
		delete this->_pDebugRenderer;
...
cDebugRenderer* cGraphicsControl::GetDebugRederer()
{
	return this->_pDebugRenderer;
}
