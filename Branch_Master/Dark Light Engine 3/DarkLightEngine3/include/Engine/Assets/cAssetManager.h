...
public:
	...
	struct sAssetpack
	{
		...
		sAssetpack(const std::string& packID)
		{
			this->cpuReady = false;
			this->gpuReady = false;
			this->_packID = packID;
			...
			InitializeCriticalSection(&this->C_CPU);
			InitializeCriticalSection(&this->C_GPU);
		}

		~sAssetpack()
		{
			DeleteCriticalSection(&(this->C_CPU));
			DeleteCriticalSection(&(this->C_GPU));
			this->Cleanup();
		}

		void SetCPUFlag(bool value)
		{
			EnterCriticalSection(&this->C_CPU);
			this->cpuReady = value;
			LeaveCriticalSection(&this->C_CPU);
		}

		void SetGPUFlag(bool value)
		{
			EnterCriticalSection(&this->C_GPU);
			this->gpuReady = value;
			LeaveCriticalSection(&this->C_GPU);
		}

		bool GetIsCPUReady()
		{
			EnterCriticalSection(&this->C_CPU);
			bool retValue = this->cpuReady;
			LeaveCriticalSection(&this->C_CPU);
			return retValue;
		}
		...
		CRITICAL_SECTION C_CPU;
		CRITICAL_SECTION C_GPU;
	};
