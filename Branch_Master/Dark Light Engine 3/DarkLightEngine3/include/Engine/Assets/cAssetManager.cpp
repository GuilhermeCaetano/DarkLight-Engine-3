...
{
	sAssetpack* assetPack = new sAssetpack(packID);
...
void cAssetManager::_CheckPendingGPULoads(cAssetManager::sAssetpack* assetPack, GLuint shaderProgramID)
{
	...
	if (ready)
	{
		...
		assetPack->SetGPUFlag(true);
	}

	LeaveCriticalSection(&(this->CS_MapLock));
}
...
void cAssetManager::_Cleanup()
{
	...
	for (unsigned int i = 0; i < this->_vecAssetPacks.size(); i++)
	{
		printf("Cleaning Asset Pack %s.\n\n", this->_vecAssetPacks[i]->_packID.c_str());
...
{
	bool result = (this->_packState == sAsset::eAssetStates::LOADED_INTO_CPU);
	return result;
}
