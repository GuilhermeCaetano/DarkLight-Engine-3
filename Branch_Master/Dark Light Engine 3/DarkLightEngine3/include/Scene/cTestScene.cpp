...
cTestScene::cTestScene(unsigned long long sceneID, std::string sceneName)
{
...
std::string cTestScene::GetSceneName()
{
...
void cTestScene::CreateScene(cVaoMeshManager* pVaoMeshManager, cEntityManager* pEntityManager, cAssimpMeshLoader* pAssimpLoader)
{
	...
	{
		sEntity* FBO = pEntityManager->GenerateEntityOfType("EmptyPrefab");
		sMeshComponent* meshC = FBO->AddComponent<sMeshComponent>();

		FBO->SetIsActive(true);
		FBO->SetName("FBO");
		FBO->SetTag("FBO");
		FBO->transform->SetWorldPosition(glm::vec3(0.0f));
		...
		this->_pSceneEntities.push_back(FBO);
	}
	{
		sEntity* light_1 = pEntityManager->GenerateEntityOfType("LightPrefab");
		this->_pSceneEntities.push_back(light_1);
	}
	{
		sEntity* castle = pEntityManager->GenerateEntityOfType("StaticPrefab");
		castle->SetName("Castle");
		castle->SetTag("Terrain");
		sMeshComponent* meshC = castle->GetComponent<sMeshComponent>();
		meshC->SetMesh("castle.ply");
		sTextureInfo castleTex;
		castleTex.texName = "angkorWatTex.bmp";
		castleTex.texStrength = 1.0f;
		meshC->vecTextures.push_back(castleTex);

		this->_pSceneEntities.push_back(castle);
	}
