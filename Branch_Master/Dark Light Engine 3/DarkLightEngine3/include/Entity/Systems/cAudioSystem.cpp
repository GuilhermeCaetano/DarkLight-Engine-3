...
{
	std::vector<void*> vecContainers = cECSPoolContainer::GetInstance()->GetPoolContainerMultiple(sAudioComponent::GetStaticType());
	for (unsigned int i = 0; i != vecContainers.size(); i++)
	{
		sMemoryChunk::sContainer* container = static_cast<sMemoryChunk::sContainer*>(vecContainers[i]);
		for (unsigned int j = 0; j < container->numInitializedElements; j++)
		{
			sAudioComponent* audioC = nMemory::GetArithmeticArrayElement<sAudioComponent>(j, container->dynamicArray, container->objSize);
			if (audioC != nullptr)
			{
