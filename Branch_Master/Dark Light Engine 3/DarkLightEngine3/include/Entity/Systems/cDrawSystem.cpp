...
void cDrawSystem::SetupFBO()
{
	this->_pFBO = new cFBO();
	std::string FBOErrorString = "";
	this->_pFBO->InitFBO(1280, 720, FBOErrorString);
	this->SetFBO(this->_pFBO);

	this->_pFBO2 = new cFBO();
	std::string FBOErrorString2 = "";
	this->_pFBO2->InitFBO(1280, 720, FBOErrorString2);
	this->SetFBO(this->_pFBO2);
}

void cDrawSystem::DeleteFBO()
{
...
// Matrix creation
glm::mat4 ObjectMatrix(glm::mat4x4& mat, glm::vec3 position, glm::quat rotation, sMeshComponent* meshComponent)
{
	// Identity (4x4 - 1) matrix
	mat = glm::mat4x4(1.0f);
...
void cDrawSystem::DrawObjects(GLuint shaderProgramID)
{
	...
	// Find all current active levels
	//cWorld* world = cEngine::GetWorld();
	//cWorld::sLevelContainer* activeLevels = world->GetActiveLevels();

	// Get mesh component containers from the memory pool.
	// There could be multiple in case the game is rendered in chunks and require multiple containers
	std::vector<void*> vecContainer = nMemory::FindAllocatedContainerMultiple(sMeshComponent::GetStaticType());
	if (vecContainer.size() <= 0)
	{
		// Nothing to draw, exit early.
		return;
	}
	
	// Camera will be needed to manage transparency, as well as two vectors to separate transparent objects to draw secondly.
	// By using two vectors, we can first setup order of drawing and then draw in sequence, maximizing contiguous memory use.
	sCameraComponent* cameraC = static_cast<sCameraComponent*>(nMemory::FindAllocatedMemoryFromEntity("CAMERA_COMPONENT", "WorldCamera", true));
	std::vector<std::pair<sEntity*, sMeshComponent*>> nonAlphaDrawEntities;
	std::vector<std::pair<sEntity*, sMeshComponent*>> alphaDrawEntities;
	for (unsigned int contIndex = 0; contIndex < vecContainer.size(); contIndex++)
	{
		sMemoryChunk::sContainer* meshContainer = static_cast<sMemoryChunk::sContainer*>(vecContainer[contIndex]);

		// Go through all mesh components in the dynamic array.
		for (unsigned int i = 0; i < meshContainer->numInitializedElements; i++)
		{
			// Use array arithmetic to find the current array element
			sMeshComponent* meshC = nMemory::GetArithmeticArrayElement<sMeshComponent>(i, meshContainer->dynamicArray, meshContainer->objSize);

			// Exit early if not set to render
			if (meshC == nullptr || !meshC->bIsVisible)
			{
				continue;
			}

			// Exit early if object parent is not active
			sEntity* entity = static_cast<sEntity*>(meshC->GetParent());
			if (!entity->GetIsActive())
			{
				continue;
			}
			
			// Add the objects to the correct vector
			if (meshC->materialDiffuse.a >= 1.0f && !meshC->bUseAlphaMap)
			{
				//this->DrawOneObject(entity, shaderProgramID);
				nonAlphaDrawEntities.push_back(std::make_pair(entity, meshC));
			}
			else
			{
				alphaDrawEntities.push_back(std::make_pair(entity, meshC));
			}
		}
	}
	
	// Sort the transparent meshes in order of distance related to the camera, if any
	if (alphaDrawEntities.size() > 0)
	{
		BubbleSort(alphaDrawEntities, cameraC->eyePosition);
	}

	// First draw all non transparent objects. This is important to avoid them being drawn on top of the transparent ones.
	// This way first the non-transparent are drawn and then the others "adjust" colour to mimic transparency.
	for (unsigned int i = 0; i < nonAlphaDrawEntities.size(); i++)
	{
		float cameraDistance = fabs(glm::distance(nonAlphaDrawEntities[i].first->GetTransform()->GetWorldPosition(), cameraC->eyePosition));
		this->DrawOneObject(nonAlphaDrawEntities[i].first, shaderProgramID);
	}
	
	// Now draw everything that has alpha in order
	for (unsigned int i = 0; i < alphaDrawEntities.size(); i++)
	{
		float cameraDistance = fabs(glm::distance(alphaDrawEntities[i].first->transform->GetWorldPosition(), cameraC->eyePosition));
		DrawCall(shaderProgramID, alphaDrawEntities[i], cameraDistance);
	}
}
...
void cDrawSystem::DrawOneObject(sEntity* entity, GLuint shaderProgramID)
{
	...
	sEntity* worldCamera = cEngine::FindEntityByName("WorldCamera");
	sCameraComponent* cameraC = worldCamera->GetComponent<sCameraComponent>();

	sMeshComponent* meshC = entity->GetComponent<sMeshComponent>();
	nPhysics::iRigidbody* rigidbodyC = entity->GetComponent<nPhysics::sSimpleRigidbodyComponent>();
	...
	if (meshC != nullptr && entity->GetIsActive())
	{
		if (entity->GetName() == "Skybox")
		{
			...
			entity->GetTransform()->SetWorldPosition(cameraC->eyePosition);
		}

		if (entity->GetTag() == "Cloth")
		{
			nPhysics::sSimpleSoftBodyComponent* softbody = dynamic_cast<nPhysics::sSimpleSoftBodyComponent*>(entity->GetComponent<nPhysics::sSimpleSoftBodyComponent>());
			sEntity* debugSphere = cEngine::FindEntityByName("DebugSphere");
			sEntity* AABB = cEngine::FindEntityByName("AABB");
			...
			for (unsigned int j = 0; j < softbody->GetNumNodes(); j++)
			{
				...
				this->DrawCall(shaderProgramID, std::make_pair(debugSphere, debugMesh), distance);
			}
			...
			this->DrawCall(shaderProgramID, std::make_pair(AABB, aabbMesh), 0.0f);
		...
		if (neuralC != nullptr)
		{
			...
			for (unsigned int i = 0; i < neuralC->detectorPoints.size(); i++)
			{
				...
				debugMesh->nonUniformScale = glm::vec3(1.0f);
				debugSphere->transform->SetWorldPosition(position);
				this->DrawCall(shaderProgramID, std::make_pair(debugSphere, debugMesh), 0.0f);
			}
		...
		DrawCall(shaderProgramID, std::make_pair(entity, meshC), cameraDistance);
	}
}
...
void cDrawSystem::DrawCall(GLuint shaderProgramID, const std::pair<sEntity*, sMeshComponent*> meshPair, float cameraDistance)
{
	//glUniform1f(this->_renderPassNumber_UniLoc, this->_renderPassNum);	// Tell shader which pass it is
	sEntity* entity = meshPair.first;
	sMeshComponent* meshC = meshPair.second;
	...
	// Create matrix, set current object matrix and send to the shader
	glm::mat4x4 matModel = glm::mat4(1.0f);
	glm::mat4 matModelInvTrans = ObjectMatrix(matModel, entity->GetTransform()->GetWorldPosition(), entity->GetTransform()->GetWorldRotation(), meshC);
	glUniformMatrix4fv(this->_matModelTrans_UniLoc, 1, GL_FALSE, glm::value_ptr(matModel));
	glUniformMatrix4fv(this->_matModelInvTrans_UniLoc, 1, GL_FALSE, glm::value_ptr(matModelInvTrans));
	...
	// Try to find a low resolution mesh to draw if the distance is too big
	...
	if (cameraDistance > 2000.0f)
	{
		...
		modelInfo = this->_pVaoMeshManager->FindDrawInfoByName(meshName);
	}
	
	if (modelInfo == nullptr)
	{
		modelInfo = meshC->meshInfo;
	}
...
{
	//cPoolManager* tempPool = cPoolManager::GetInstance();
	sEntity* collider = cEngine::FindEntityByName(colName, false);
	...
	this->DrawCall(shaderProgramID, std::make_pair(collider, colliderMesh), 0.0f);
...
void cDrawSystem::DrawCall1Pass(const glm::vec3& renderPos, const glm::vec3& renderLookAt, int width, int height, GLuint shaderProgramID, double deltaTime)
{
	// *****************************************
	// 1st pass:
	//
...
void cDrawSystem::DrawCall2Pass(sEntity* FBOEntity, int width, int height, GLuint shaderProgramID)
{
	// *****************************************
	// 2nd pass:
	//
	...
	sMeshComponent* renderMesh = FBOEntity->GetComponent<sMeshComponent>();
	//FBOEntity->SetTag("FBO");
	FBOEntity->SetIsActive(true);
	...
	sEntity* camera = cEngine::FindEntityByName("WorldCamera");
	...
	this->DrawCall(shaderProgramID, std::make_pair(FBOEntity, renderMesh), 0.0f);
}

void cDrawSystem::DrawWireframeLightSpheres(GLuint shaderProgramID)
{
	//cPoolManager* tempPool = cPoolManager::GetInstance();
	std::vector<void*> vecContainer = nMemory::FindAllocatedContainerMultiple(sLightComponent::GetStaticType());
	if (vecContainer.size() <= 0)
	{
		// Nothing to draw, exit early.
		return;
	...
	sEntity* wireframeEntity = cEngine::FindEntityByName("WireframeSphere");
	sMeshComponent* wireMeshC = wireframeEntity->GetComponent<sMeshComponent>();
	for (unsigned int contIndex = 0; contIndex < vecContainer.size(); contIndex++)
	{
		sMemoryChunk::sContainer* lightContainer = static_cast<sMemoryChunk::sContainer*>(vecContainer[contIndex]);

		for (int i = 0; i != lightContainer->arraySize; i++)
		{
			// Use array arithmetic to find the current array element
			sLightComponent* lightC = nMemory::GetArithmeticArrayElement<sLightComponent>(i, lightContainer->dynamicArray, lightContainer->objSize);

			// Exit early if not set to render
			if (lightC == nullptr || !lightC->GetIsLightOn() || lightC->bisWireframeOn == false)
			{
				continue;
			}

			// Exit early if object parent is not active
			sEntity* entity = static_cast<sEntity*>(lightC->GetParent());
			if (!entity->GetIsActive())
			{
				continue;
			}
			...
			wireframeEntity->GetTransform()->SetWorldPosition(glm::vec3(lightC->position));
			...
			float distance90Percent = this->_pLightHelper->calcApproxDistFromAtten(0.65f, ACCURACY_OF_DISTANCE, INFINITE_DISTANCE,
				lightC->attenuation.x,
				lightC->attenuation.y,
				lightC->attenuation.z);

			wireMeshC->nonUniformScale = glm::vec3(distance90Percent);		// 90% brightness
			wireMeshC->materialDiffuse = glm::vec4(1.0f, 1.0f, 0.0f, 1.0f);
			DrawCall(shaderProgramID, std::make_pair(wireframeEntity, wireMeshC), 0.0f);
...
void cDrawSystem::DrawAABBs(cAABBManager* pAABBManager, GLuint shaderProgramID)
{
	for (std::map<unsigned long long, cAABB*>::iterator mapIter = pAABBManager->_pMapAABB.begin(); mapIter != pAABBManager->_pMapAABB.end(); mapIter++)
	{
		...
		//cPoolManager* tempPool = cPoolManager::GetInstance();
		sEntity* AABBEntity = cEngine::FindEntityByName("AABB", false);
		...
		AABBEntity->GetTransform()->SetWorldPosition(mapIter->second->GetCenterXYZ());
		AABBMeshC->nonUniformScale = glm::vec3(mapIter->second->GetSideLength());
		AABBMeshC->bIsWireframe = true;
		AABBMeshC->bIsVisible = true;

		DrawCall(shaderProgramID, std::make_pair(AABBEntity, AABBMeshC), 0.0f);
...
void cDrawSystem::DrawCollider(sEntity* entity, GLuint shaderProgramID)
{
	...
	if (pointsColliderC != nullptr)
	{
		...
		{
			//cPoolManager* tempPool = cPoolManager::GetInstance();
			sEntity* debugEntity = cEngine::FindEntityByName("DebugSphere");
			sMeshComponent* debugMeshC = debugEntity->GetComponent<sMeshComponent>();
			...
			debugEntity->GetTransform()->SetWorldPosition(worldTransform);
			meshC->nonUniformScale = glm::vec3(0.3f);
			debugMeshC->materialDiffuse = glm::vec4(0.0f, 0.0f, 1.0f, 1.0f);
			debugMeshC->bIsVisible = true;

			DrawCall(shaderProgramID, std::make_pair(debugEntity, debugMeshC), 0.0f);

			debugEntity->GetTransform()->SetWorldPosition(glm::vec3(0.0f));
			meshC->nonUniformScale = glm::vec3(1.0f);
			debugMeshC->materialDiffuse = glm::vec4(1.0f);
			debugMeshC->bIsVisible = false;
		}
	}
}
