...
{
	for (unsigned long i = 0; i < this->_pTexTripleThread->vecsTextures[this->_pTexTripleThread->GetCurrentReadBuffer()].size(); i++)
	{
		std::pair<std::string, cTextureBMP*> readPair = this->_pTexTripleThread->vecsTextures[this->_pTexTripleThread->GetCurrentReadBuffer()][i];

		if (readPair.second == nullptr)
		{
			continue;
		}

		std::pair<std::string, cTextureBMP*> writePair = this->_pTexTripleThread->vecsTextures[this->_pTexTripleThread->GetCurrentWriteBuffer()][i];

		// Is this texture ready to be sent to the GPU?
		if (readPair.second->GetTextureInfo()->_texState == cTextureBMP::LOADED_FROM_FILE)
		{
			// Load this texture on the GPU
			this->_LoadPendingTextureIntoGPU(readPair.second);
			readPair.second->GetTextureInfo()->_texState = cTextureBMP::LOADED_IN_GPU;
		}
		else if (readPair.second->GetTextureInfo()->_texState == cTextureBMP::LOADED_IN_GPU)
		{
			// Is this texture all ready to be set on the map?
			// Set this texture into the high res map and set state to ready
			readPair.second->GetTextureInfo()->_texState = cTextureBMP::READY_TO_USE;
			this->_mapHighResTexByName[readPair.first] = readPair.second;
		}
	}
