...
DWORD WINAPI TLoadTexture(PVOID pvParam)
{
	...
	pTextureBMP->LoadTextureFromCPU(pTextureBMP->GetTextureFullPathName());
	...
	while (pTextureBMP->GetTextureInfo()->_texState != cTextureBMP::LOADED_FROM_FILE)
	{
		pTextureBMP->UpdateTripleBuffer();
	}
...
bool cTextureBMP::CreateTextureThread(cTextureTripleBuffer* pTexTripleBuffer)
{
	...
	{
		return false;
		printf("Warning! Thread not created successfully, but buggy vs can't throw false here...\n");
	}
...
void cTextureBMP::UpdateTripleBuffer()
{
	if (this->_pTexTripleBuffer->vecsTextures[this->_pTexTripleBuffer->GetCurrentWriteBuffer()][this->_pTexInfo->_texIndex].second == nullptr)
	{
		...
		this->_pTexTripleBuffer->vecsTextures[this->_pTexTripleBuffer->GetCurrentWriteBuffer()][this->_pTexInfo->_texIndex].second = this;
	}
	else
	{
		this->_pTexInfo->_texState = eTexState::LOADED_FROM_FILE;
	}
}
