...
bool cSTLVector::LoadDataFilesIntoContainer(std::string firstNameFemaleFileName, std::string firstNameMaleFileName, std::string lastNameFileName)
{
	...
	for (unsigned int i = 0; i < 400; i += 4)
	{
		...
		this->_vecPeople.push_back(person);
	}
...
bool cSTLVector::FindPeopleByName(std::vector<sPerson>& vecPeople, sPerson personToMatch, int maxNumberOfPeople)
{
	...
	for (unsigned int i = 0; i < this->_vecPeople.size(); i++)
	{
		sPerson* person = this->_vecPeople[i];
...
bool cSTLVector::FindPeopleByName(std::vector<sPerson>& vecPeople, std::vector<sPerson>& vecPeopleToMatch, int maxNumberOfPeople)
{
	...
	for (unsigned int i = 0; i < this->_vecPeople.size(); i++)
	{
		sPerson* pPerson = this->_vecPeople[i];
...
bool cSTLVector::FindPeople(std::vector<sPerson>& vecPeople, float minHealth, float maxHealth, int maxPeopleToReturn)
{
	int numPeopleFound = 0;
	for (unsigned int i = 0; i < this->_vecPeople.size(); i++)
	{
...
bool cSTLVector::FindPeople(std::vector<sPerson>& vecPeople, sPoint location, float radius, float minHealth, float maxHealth, int maxPeopleToReturn)
{
	int numPeopleFound = 0;
	for (unsigned int i = 0; i < this->_vecPeople.size(); i++)
	{
